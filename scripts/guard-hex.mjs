...
/*
  tokens.css owns colour. Two files are exempt and each has a reason that is not "it was
  inconvenient":

    apps/web/public/favicon.svg   a browser tab icon is a static asset loaded outside the
                                  document, so it cannot read a custom property. It is
                                  GENERATED by scripts/build-favicon.mjs from the same glyph
                                  the mark uses, so the shape cannot drift; only the two fill
                                  values live here.

    apps/web/index.html           theme-color is read by the OS chrome before any stylesheet
                                  exists. A var() here resolves to nothing and the browser
                                  falls back to white, which is the one flash a dark app
                                  cannot afford.

  Both are listed rather than pattern matched, so adding a third is a visible act.
*/
const ALLOW = new Set([
